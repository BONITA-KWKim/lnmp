...
def compute_metrics(label_list, predicted_label_list):
  ...
  print('[D] tn: {}, fp:{}, fn: {}, tp: {}'.format(tn, fp, fn, tp))
...
def train_val_test_binary_class(task_type, epoch, model, data_loader, optimizer, 
                                merge_method, logger=None):
  total_loss = 0
  label_list = []
  ...
  if task_type == "train":
    ...
    for index, item in enumerate(data_loader, start=1):
      ...
      clinical_data = item["clinical_data"][0].cuda() \
        if "clinical_data" in item else None
      
      if clinical_data is not None:
        clinical_data = clinical_data[1:] # remove metastasis
  ...
  if logger == None:
    ...
    logger.info(
      f"epoch: {epoch}, {task_type}, loss: {average_loss:.3f}, threshold: {threshold}, auc: {metrics_dict['auc']:.3f}, acc: {metrics_dict['acc']:.3f}, sens: {metrics_dict['sens']:.3f}, spec: {metrics_dict['spec']:.3f}, ppv: {metrics_dict['ppv']:.3f}, npv: {metrics_dict['npv']:.3f}, f1: {metrics_dict['f1']:.3f}"
    )
  return metrics_dict["auc"]


def train_val_test_multi_class(task_type, epoch, model, data_loader, optimizer, 
                               merge_method, logger=None):
  total_loss = 0
  label_list = []
  ...
  if logger == None:
    ...
    logger.info(f"epoch: {epoch}, {task_type}, loss: {average_loss:.3f}, auc: {auc:.3f}")
