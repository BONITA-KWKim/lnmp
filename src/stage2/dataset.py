...
class MILDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, json_path, data_dir_path='./dataset', \
               clinical_data_path=None, is_preloading=True):
    ...
    if clinical_data_path is not None:
      ...
      self.clinical_data_df = pd.read_excel(
        clinical_data_path, index_col="p_id", engine="openpyxl"
      )
    else:
  ...
  def __getitem__(self, index):
    ...
    if self.clinical_data_df is not None:
      ...
      # data["clinical_data"] = self.clinical_data_df[patient_id].to_numpy()
