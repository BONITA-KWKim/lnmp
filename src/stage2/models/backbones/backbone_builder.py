...
class BackboneBuilder(nn.Module):
  ...
  def __init__(self, backbone_name):
    ...
    # These backbone has multiple output
    if "googlenet" in backbone_name or "inception_v3" in backbone_name: 
    ...
    if backbone_name.startswith("vgg"):
      ...
      self.extractor, self.output_features_size = \
        self.resnet(backbone_name, complete_backbone)
    elif backbone_name.startswith("densenet"):
      assert backbone_name in [
        "densenet121", "densenet161", "densenet169", "densenet201"
      ]
      self.extractor, self.output_features_size = \
        self.densenet(backbone_name, complete_backbone)
    elif backbone_name == "googlenet":
      self.extractor, self.output_features_size = \
        self.googlenet(complete_backbone)
    elif backbone_name == "inception_v3":
      self.extractor, self.output_features_size = \
        self.inception_v3(complete_backbone)
    elif backbone_name == "alexnet":
      self.extractor, self.output_features_size = \
        self.alexnet(complete_backbone)
    elif backbone_name == "resnext50_32x4d":
      self.extractor, self.output_features_size = \
        self.resnext(complete_backbone)
    else:
  ...
  def resnext(self, complete_backbone):
    output_features_size = 1024 * 1 * 1
    extractor = nn.Sequential(*(list(complete_backbone.children())[:-1]))
